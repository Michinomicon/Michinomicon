...
import { type AudioStore, calculateNextIndex, canUseDOM, useAudioStore } from '@/lib/audio-store'
 ...
 */
const getErrorInfo = (errorCode: number): { message: string; recoverable: boolean } => {
  switch (errorCode) {
...
function AudioProvider({ tracks = [], children }: { tracks?: Track[]; children: React.ReactNode }) {
  ...
  const setState = React.useCallback(
    (partial: Partial<AudioStore> | ((state: AudioStore) => Partial<AudioStore>)) => {
  ...
  React.useEffect(() => {
    if (tracks && tracks.length > 0) {
      ...
      if (tracksChanged) {
        console.debug(`AudioProvider => tracksChanged`, tracks)
        const firstTrack = tracks[0]
        useAudioStore.setState({
          queue: tracks,
          currentTrack: state.currentTrack || firstTrack,
          currentQueueIndex: state.currentQueueIndex === -1 ? 0 : state.currentQueueIndex,
        })
      }
    }
  }, [tracks])
  ...
  const retryPlayback = React.useCallback(
    async (audio: HTMLAudioElement) => {
      ...
      try {
        console.log(`Retry attempt ${errorRetryCountRef.current}: Loading audio...`)
  ...
  React.useEffect(() => {
    ...
    const handleError = async (e: Event) => {
      const { message: initialMessage, recoverable } = parseAudioError(e, audio)
  ...
  }, [throttledTimeUpdate, setState, retryPlayback, preloadNextTrack, htmlAudio, forceTimeUpdate])
