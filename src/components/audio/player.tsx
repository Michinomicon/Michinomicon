...
import { AudioSlider } from '@/components/ui/slider'
...
function AudioPlayerButton({ tooltip = false, tooltipLabel, ...props }: AudioPlayerButtonProps) {
...
function AudioPlayer({ children, className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn('w-full rounded-lg border bg-card p-1.5', className)}
...
const AudioPlayerTimeDisplay = ({
  ...
  const formattedCurrentTime = React.useMemo(() => formatDuration(currentTime), [currentTime])
  ...
  const displayTime = isMounted ? timeValue : '0:00'
...
const AudioPlayerSeekBar = ({
  ...
  React.ComponentProps<typeof AudioSlider>,
...
const AudioPlayerVolume = ({
  ...
  variant = 'outline',
  ...props
}: Omit<React.ComponentProps<typeof AudioSlider>, 'value' | 'onValueChange' | 'min' | 'max'> & {
  ...
  const getIcon = () => {
    return <Icon className={cn(isMuted && 'opacity-40', 'text-primary')} />
  }
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <AudioPlayerButton
          ...
          size={size}
          tooltip
          tooltipLabel={isMuted ? 'Muted' : `Volume ${Math.round(volumePercent)}%`}
          variant={variant}
        >
          {getIcon()}
        </AudioPlayerButton>
      </DropdownMenuTrigger>
      <DropdownMenuContent className={cn('flex w-48 flex-col gap-1.5 p-1.5', className)}>
        <div className="flex items-center justify-between">
          <span className="text-sm">Volume</span>
          <span className="font-mono text-sm tabular-nums">{volumePercent}%</span>
        </div>
        <div className="flex items-center gap-2">
          <VolumeXIcon
            aria-hidden="true"
            className={cn('size-4 shrink-0 cursor-pointer', isMuted ? 'opacity-40' : 'opacity-60')}
            onClick={toggleMute}
            role="button"
          />
          <AudioSlider
            ...
            {...props}
          />
          <Volume2Icon aria-hidden="true" className="size-4 shrink-0 opacity-60" />
        </div>
      </DropdownMenuContent>
    </DropdownMenu>
  )
...
const AudioPlayerPlay = React.memo(
  ({
    ...
    const isDisabled = isMounted ? showSpinner || !currentTrack : false
...
const AudioPlayerRewind = React.memo(
  ({
    ...
    const isDisabled = isMounted ? disableSeekBackward : false
