...
import { ListMusicIcon, MusicIcon, PauseIcon, PlayIcon, RadioIcon, XIcon } from 'lucide-react'
      ...
      <div className="flex items-center gap-2">
        ...
        {coverImage ? (
          <Avatar className="rounded-sm">
            <AvatarImage alt={track.title} className="object-cover" src={coverImage} />
...
function AudioTrack({
  ...
  return (
    ...
    >
      <ItemMedia>{renderTrackMedia(showDragHandle, showCover, track, index)}</ItemMedia>
      <ItemContent className="min-w-0 flex-1 gap-0 overflow-hidden">
        <div className="flex items-center gap-1.5">
          <ItemTitle className="truncate font-medium text-sm">{track.title}</ItemTitle>
          {isLiveTrack && (
      ...
      <ItemActions>
        {showRemove && !isCurrent && onRemove && (
          <Button onClick={handleRemove} size="icon" title="Remove" variant="ghost">
            <XIcon />
          </Button>
        )}
        {showPlayPause && (
...
function AudioTrackList({
    ...
    return (
      <Empty className={cn('mx-auto size-full border bg-muted/30', className)}>
        <EmptyHeader>
          ...
          <EmptyDescription className="text-xs/relaxed">{emptyDescription}</EmptyDescription>
        </EmptyHeader>
      </Empty>
    )
  }
  ...
  const content = sortable ? (
    <SortableList
      ...
      items={tracks.filter((t) => t.id !== undefined).map((t) => ({ id: String(t.id) }))}
  ...
  return (
    <ScrollArea className={cn('w-full scroll-pt-2 scroll-pb-1.5 pt-1', className)}>
      {content}
    </ScrollArea>
  )
}
