import { Right } from "@/payload-types";
import type { Access } from "payload";

type AccessType = 'create' | 'read' | 'upd' | 'del';

export const hasAccess = (collection: string, access_type: AccessType): Access => async ({ req, req: { user, payload } }) => {
    console.log(user)
        ...
        if (user.role && access_type) {
            ...
            if (rights.totalDocs > 0) {
                ...
                const collection_rights = rights_list?.filter((right) => {
                    return right.collections?.filter((col) => typeof col === "string" ? col === collecID : col.id === collecID).length || 0 > 0;
                });
                if (collection_rights && collection_rights.length > 0) {
                    ...
                    if (access_type === 'read') {
                        ...
                        return right_own
                            && canAccessOthersOrSelf(Boolean(right_others), user.id);
                    }
                    if (access_type === 'upd') {
                    ...
                    if (access_type === 'del') {
                        ...
                        console.log('delete access', right_own && canAccessOthersOrSelf(Boolean(right_others), user.id));
                        return right_own
                            && canAccessOthersOrSelf(Boolean(right_others), user.id);
                    }
                }
